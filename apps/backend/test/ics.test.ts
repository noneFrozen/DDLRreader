...
describe("ICS export", () => {
  ...
  it("escapes task titles and keeps block UIDs stable across exports", async () => {
    ...
    await app.inject({ method: "POST", url: "/api/tasks", headers: { cookie }, payload: { title: `软件工程\\,大;作业\r\n下一行${"界".repeat(30)}`, deadline: "2026-08-11T12:00:00.000Z", remainingMinutes: 30 } });
    await app.inject({ method: "POST", url: "/api/plans", headers: { cookie }, payload: { planningDays: 7, allowRisk: false } });

    const exported = await app.inject({ method: "GET", url: "/api/plans/plan-1/export.ics", headers: { cookie } });
    const exportAgain = await app.inject({ method: "GET", url: "/api/plans/plan-1/export.ics", headers: { cookie } });
  ...
  it("returns structured missing-plan and missing-task errors and exports the requested old version", async () => {
    ...
    const cookie = seedSession(database);
    await app.inject({ method: "POST", url: "/api/tasks", headers: { cookie }, payload: { title: "Old task", deadline: "2026-08-11T12:00:00.000Z", remainingMinutes: 30 } });
    ...
    repository.savePlan(USER, { ...base, id: "old", version: 1, blocks: [{ id: "old-block", taskId: "task-1", startAt: "2026-08-10T09:00:00.000Z", endAt: "2026-08-10T09:30:00.000Z", status: "planned", locked: false }] });
    repository.savePlan(USER, { ...base, id: "latest", version: 2, blocks: [{ id: "latest-block", taskId: "task-1", startAt: "2026-08-10T10:00:00.000Z", endAt: "2026-08-10T10:30:00.000Z", status: "planned", locked: false }] });
    expect((await app.inject({ method: "GET", url: "/api/plans/missing/export.ics", headers: { cookie } })).json()).toEqual({ code: "PLAN_NOT_FOUND", message: "计划不存在" });
    const oldExport = await app.inject({ method: "GET", url: "/api/plans/old/export.ics", headers: { cookie } });
    ...
    const missingTask = await app.inject({ method: "GET", url: "/api/plans/old/export.ics", headers: { cookie } });
