...
function toBlock(row: BlockRow): ScheduleBlock {
  return { id: row.id, taskId: row.task_id, startAt: row.start_at, endAt: row.end_at, status: row.status, locked: row.locked === 1 };
}
...
export class SqlitePlanRepository implements PlanRepository {
  ...
  updateBlock(userId: string, planId: string, block: ScheduleBlock): void {
    ...
    const overlaps = this.database.prepare(`
      ...
      LIMIT 1
    `).get(planId, block.id, block.endAt, block.startAt);
